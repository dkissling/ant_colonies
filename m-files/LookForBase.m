% Folgt einer Spur auf Phero_Field_Base
% Gibt phi zurück
...
if max(dir) < n && min(dir) > 1
    ...
    if dir(1) == location(1) %nach rechts oder links
        ...
        for i = -1:1
            vector = vector + Phero_Field_Base(dir(1),dir(2)+i)*[i;dir(2)-location(2)];
        end
        
    elseif dir(2) == location(2) %nach oben oder unten
        
        for i = -1:1
            vector = vector + Phero_Field_Base(dir(1)+i,dir(2))*[dir(1)-location(1);i];
        end
        ...
        for i = 0:1
            for j = 0:1
                if i ~= 0 || j ~=0
                    vector = vector + Phero_Field_Base(location(1)+i*rel(1),location(2)+j*rel(2))*[i*rel(1);j*rel(2)]/norm([i*rel(1);j*rel(2)]);
                end
            end
        end
    end
